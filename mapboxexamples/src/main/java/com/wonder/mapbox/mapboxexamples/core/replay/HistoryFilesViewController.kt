package com.wonder.mapbox.mapboxexamples.core.replay
...
import com.wonder.mapbox.mapboxexamples.core.replay.ReplayDataSource.ASSETS_DIRECTORY
import com.wonder.mapbox.mapboxexamples.core.replay.ReplayDataSource.HISTORY_RECORDER
...
class HistoryFilesViewController {
    ...
    private suspend fun requestHistoryDisk(
        ...
        historyFiles.filter { it.endsWith(".json") }
            .map { fileName ->
                ReplayPath(
                    title = context.getString(string.history_local_history_file),
                    description = fileName,
                    path = fileName,
                    dataSource = ASSETS_DIRECTORY
                )
            }
    }
    ...
    private suspend fun requestHistoryCache(
        ...
        historyFiles.map { file ->
            ReplayPath(
                title = context.getString(string.history_recorded_history_file),
                description = file.name,
                path = file.absolutePath,
                dataSource = HISTORY_RECORDER
            )
        }
    }
